...
<header id="header" class="header fixed-top d-flex align-items-center">
    ...
    <div class="d-flex align-items-center justify-content-between m-4 ">
        <a href="main" class="logo d-flex align-items-center">
            <img src="<?= base_url() ?>NiceAdmin/assets/img/Newbalance-logo.webp" alt="">
        </a>
    ...
    <nav class="header-nav ms-auto">
        <ul class="d-flex align-items-center">
            ...
            <!-- Cart -->
            <li class="nav-item">
                <a class="nav-link nav-icon" href="<?= base_url('keranjang') ?>">
                <i class="bi bi-cart"></i>
            <?php 
                $cart = session()->get('keranjang') ?? [];
                $cart_count = is_array($cart) ? array_sum(array_column($cart, 'qty')) : 0;
            ?>
            <span class="badge bg-danger badge-number"><?= $cart_count ?></span>
            </a>
            </li>
            ...
            <li class="nav-item dropdown pe-3">
                <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown">
                    <img src="<?= base_url() ?>NiceAdmin/assets/img/self 1.jpg" alt="Profile" class="rounded-pill" width="40px">
                    <span class="d-none d-md-block dropdown-toggle ps-2"><?= session()->get('username'); ?> (<?= session()->get('role'); ?>)</span>
                </a>
                <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile">
                    <li class="dropdown-header">
                        <h6><?= session()->get('username'); ?></h6>
                        <span class="text-danger"><?= session()->get('role'); ?></span>
                    </li>
                    <li><hr class="dropdown-divider"></li>
                    <li>
